...
canBuild=["$TRAVIS_REPO_SLUG" -eq "Valkryst/VTerminal"]
canBuild&=["$TRAVIS_JDK_VERSION" -eq "oraclejdk8"]
canBuild&=["$TRAVIS_PULL_REQUEST" -eq "false"]
canBuild&=["$TRAVIS_BRANCH" -eq "master"]
